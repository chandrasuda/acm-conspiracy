...
class Round(models.Model):
    ...
    selection_stage = models.BooleanField(default=False, verbose_name=_('Selection status'),
                                        help_text=_('Used to denote whether we select novice or advanced here.'))
    
    display_locked = models.BooleanField(default=True, verbose_name=_('Locked Puzzle Visibility'),
                                         help_text=_('Used to denote whether we display locked fields.'))
...
class UnlockGroup(models.Model):
    name = models.CharField(max_length=255, verbose_name=_('Name of the unlock group'))
    count = models.IntegerField(verbose_name=_('Number of puzzles in this unlock group'))
...
class Puzzle(models.Model):
    ...
    prompts_on = models.BooleanField(
        default=False, verbose_name=_('Has prompt'),
        help_text=_('Does this puzzle have prompting?')
    )

    prompt = models.CharField(
        default='No prompt', max_length=255, verbose_name=_('Prompt'),
        help_text=_('Prompt to tell people where to go during runaround.')
    )

    hints_on = models.BooleanField(
        default=True, verbose_name=_('Has hints'),
        help_text=_('Does this puzzle have hints?')
    )

    hint_one = models.CharField(
        default='No hint', max_length=255, verbose_name=_('First Hint'),
        help_text=_('Hint to be unlocked at 15 minutes after the puzzle starts'),
    )

    hint_two = models.CharField(
        default='No hint', max_length=255, verbose_name=_('Second Hint'),
        help_text=_('Hint to be unlocked at 25 minutes after the puzzle starts'),
    )

    hint_three = models.CharField(
        default='No hint', max_length=255, verbose_name=_('Third Hint'),
        help_text=_('Hint to be unlocked at 35 minutes after the puzzle starts'),
    )

    novice_hints = models.BooleanField(
        default=False, verbose_name=_('Has novice hints?'),
        help_text=_('Whether or not novice hints should be considered for this puzzle'),
    )

    hint_one_novice = models.CharField(
        default='No hint', max_length=255, verbose_name=_('First Hint'),
        help_text=_('Novice hint to be unlocked at 15 minutes after the puzzle starts'),
    )

    hint_two_novice = models.CharField(
        default='No hint', max_length=255, verbose_name=_('Second Hint'),
        help_text=_('Novice hint to be unlocked at 25 minutes after the puzzle starts'),
    )

    hint_three_novice = models.CharField(
        default='No hint', max_length=255, verbose_name=_('Third Hint'),
        help_text=_('Novice hint to be unlocked at 35 minutes after the puzzle starts'),
    )
    ...
    unlock_group = models.ForeignKey(UnlockGroup, on_delete=models.CASCADE, verbose_name=_('unlock group'))
    order = models.IntegerField(default=0, verbose_name=_('Order'))
...
class Preamble(models.Model):
    ...
    slug = models.SlugField(
        max_length=255, unique=True, verbose_name=_('Slug'),
        help_text=_('Slug used in URLs to identify this preamble (must be unique)'),
    )
    ...
    class Meta:
        ...
        verbose_name_plural = _('preambles')
...
class Team(models.Model):
    ...
    division = models.IntegerField(default=0, verbose_name=_('Team division'), help_text=_('''The division of the team, numbers less than or equal to one
    are advanced division team members, numbers greater than one are beginner division team members.'''))
    ...
    finished = models.BooleanField(default=False, verbose_name=_('Has the user finished the problem?'))
    time_so_far = models.IntegerField(default=0, verbose_name=_('Total number of milliseconds in user solve time'))
    ...
    def preamble_completions(self, preamble):
        return [
            completion for completion in self.completions
            if completion.preamble == preamble
        ]
    ...
    def preamble_submit(self, preamble):
        for completion in self.completions:
            if preamble == completion.preamble:
                return preamble.id
        return None
    ...
    def guesses_remaining(self, puzzle):
        wrong_guesses = sum(
            ...
            if not submission.is_correct and (timezone.make_aware(datetime.datetime.now()) - submission.submitted_datetime).total_seconds() < 60
        )
    ...
    def completions(self):
        return tuple(
            self.preamblecompletion_set
            .select_related('preamble', 'preamble__round')
            .order_by('-submitted_datetime')
        )
    ...
    def preamble_solves(self):
        return {
            submission.preamble_id: submission.preamble
            for submission in self.completions
        }
    ...
    def db_unlocks(self):
        ...
        for unlock in self.preambleunlock_set.select_related('preamble', 'preamble__round'):
            unlockset[str(unlock.preamble_id) + "_preamble"] = unlock
    ...
    def main_round_solves(self):
        ...
        for preamble in self.preamble_solves.values():
            local_preamble[preamble.round.slug] += 1
            if preamble.round.slug == INTRO_ROUND_SLUG:
                continue
        return (global_solves, local_solves, local_preamble)
    ...
    def compute_unlocks(context):
        ...
        unlocks_puzzle = []
        unlocks_preamble = []
        for preamble in context.all_preambles:
            unlocked_at = None
            if 0 <= preamble.unlock_hours and (
                preamble.unlock_hours == 0 or
                not context.team or
                context.team.allow_time_unlocks):
                unlock_time = context.start_time + datetime.timedelta(hours=preamble.unlock_hours)
            ...
            if context.hunt_is_prereleased or context.hunt_is_over:
                ...
                (global_solves, local_solves, local_preamble) = context.team.main_round_solves
                if 0 <= preamble.unlock_global <= global_solves and ((global_solves is not None) or any(metas_solved)):
                    unlocked_at = context.now
                if (str(preamble.id) + "_preamble") in context.team.db_unlocks:
                    unlocked_at = context.team.db_unlocks[str(preamble.id) + "_preamble"].unlock_datetime
                elif unlocked_at:
                    unlocks_preamble.append(Team.unlock_preamble(context, preamble, unlocked_at))
            if unlocked_at:
                puzzles_unlocked[preamble] = unlocked_at

        for puzzle in context.all_puzzles:
            unlocked_at = None

            if not (context.team and context.team.division != 0) and puzzle.round.selection_stage:
                continue
            ...
            # conditional on other puzzles
            elif context.team:
                (global_solves, local_solves, local_preamble) = context.team.main_round_solves
                if local_preamble[puzzle.round.slug] > 0:
                    if 0 <= puzzle.unlock_global <= global_solves and (puzzle.unlock_global == 0 or global_solves or any(metas_solved)):
                        unlocked_at = context.now
                    ...
                    if puzzle.id in context.team.db_unlocks:
                        ...
                        unlocks_puzzle.append(Team.unlock_puzzle(context, puzzle, unlocked_at))

            if unlocked_at:
                puzzles_unlocked[puzzle] = unlocked_at
        if unlocks_puzzle:
            PuzzleUnlock.objects.bulk_create(unlocks_puzzle, ignore_conflicts=True)
        if unlocks_preamble:
            PreambleUnlock.objects.bulk_create(unlocks_preamble, ignore_conflicts=True)
    ...
    @staticmethod
    def unlock_preamble(context, preamble, unlocked_at):
        unlock = PreambleUnlock(
            team=context.team,
            preamble=preamble,
            unlock_datetime=unlocked_at)
        context.team.db_unlocks[str(preamble.id) + "_preamble"] = unlock
        return unlock

    def intermediate_confirmations(context, puzzle):
        confirmations = IntermediateConfirmation.objects.filter(
            puzzle=puzzle
        )
        # print(confirmations)
        inputs = []
        for x in list(confirmations):
            inputs.append(x.input_text.upper())
        return inputs
...
class PreambleUnlock(models.Model):
    '''Represents a team having access to a preamble (and when that occurred).'''

    team = models.ForeignKey(Team, on_delete=models.CASCADE, verbose_name=_('team'))
    preamble = models.ForeignKey(Preamble, on_delete=models.CASCADE, verbose_name=_('preamble'))
    ...
    def __str__(self):
        return '%s -> %s @ %s' % (
            self.team, self.preamble, self.unlock_datetime
        )

    class Meta:
        unique_together = ('team', 'preamble')
        verbose_name = _('preamble unlock')
        verbose_name_plural = _('preamble unlocks')

class PuzzleFirstViewTime(models.Model):
    team = models.ForeignKey(Team, on_delete=models.CASCADE, verbose_name=_('team'))
    unlock_group = models.ForeignKey(UnlockGroup, on_delete=models.CASCADE, verbose_name=_('unlock group'))
    solve_count = models.IntegerField(default=0, verbose_name=_('Count of puzzles solved in the solve group'))
    first_view_time = models.DateTimeField(verbose_name=_('First view by team datetime'))
    fullsolve_time = models.DateTimeField(null=True, verbose_name=_('Time all puzzles in the unlock group are solved'))
    
    class Meta:
        unique_together = ('team', 'unlock_group')
        verbose_name = _('team unlock group time')
        verbose_name_plural = _('team unlock group times')

class IntermediateConfirmation(models.Model):
    puzzle = models.ForeignKey(Puzzle, on_delete=models.CASCADE, verbose_name=_('puzzle'))
    input_text = models.CharField(max_length=255, verbose_name=_('Input text'))
    feedback_text = models.CharField(max_length=255, verbose_name=_('Feedback'))
...
class PreambleCompletion(models.Model):
    '''Represents a team making a solve attempt on a puzzle (right or wrong).'''
    ...
    def __str__(self):
        return '%s -> %s' % (
            self.team, self.preamble
        )

    class Meta:
        unique_together = ('team', 'preamble')
        verbose_name = _('preamble completion')
        verbose_name_plural = _('preamble completions')
