...
def basic_context_middleware(get_response):
    def middleware(request):
        request.context = BasicContext(request)
...
class Context:
    ...
    def all_preambles(self):
        return tuple(models.Preamble.objects.select_related('round').order_by('round__order', 'order'))
    ...
    def first_hint(self):
        return -1 # set by middleware

    def puzzle_answer(self):
        return self.team and self.puzzle and self.team.puzzle_answer(self.puzzle)

    def preamble_completed(self):
        return self.team and self.preamble and self.team.preamble_submit(self.preamble)
    ...
    def intermediate_confirmations(self):
        # print("INTERMEDIATE:", self.team and self.puzzle and self.team.intermediate_confirmations(self.puzzle))
        return self.team and self.puzzle and self.team.intermediate_confirmations(self.puzzle)

    def round(self):
        if self.puzzle:
            return self.puzzle.round
        elif self.preamble:
            return self.preamble.round
        else:
            return None
