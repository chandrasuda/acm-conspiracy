...
def validate_puzzle(require_team=False):
    ...
    def decorator(f):
        ...
        def inner(request, slug):
            ...
            preamble = Preamble.objects.select_related().filter(slug=slug).first()
            if preamble:
                return redirect('preamble', slug)
...
def validate_preamble(require_team=False):
    '''
    Indicates an endpoint that takes a single URL parameter, the slug for the
    preamble. If the slug is invalid, report an error and redirect to /puzzles.
    If require_team is true, then the user must also be logged in.
    '''
    def decorator(f):
        @wraps(f)
        def inner(request, slug):
            preamble = Preamble.objects.select_related().filter(slug=slug).first()
            request.context.preamble = preamble
            if not preamble or preamble not in request.context.unlocks:
                messages.error(request, _('Invalid preamble name.'))
                return redirect('puzzles')
            if request.context.team:
                unlock = request.context.team.db_unlocks.get(str(preamble.id) + "_preamble")
                if unlock and not unlock.view_datetime:
                    unlock.view_datetime = request.context.now
                    unlock.save()
            elif require_team:
...
def puzzles(request):
    ...
    if request.context.hunt_has_started:
        ...
        round_ids = []
        for r in render_puzzles(request).values():
            round_ids.append(r['round'])
        # print("ROUND IDS:", round_ids)
        return render(request, 'puzzles.html', {'rounds': render_puzzles(request), 'all_rounds': get_all_rounds(request), 'round_helper': round_ids})
...
# {'round': <Round: Round 2>, 
# 'puzzles': [{'puzzle': <Preamble: preamble2>, 'full_stats': False, 'new': False}, 
#             {'puzzle': <Puzzle: Round2P1>, 'full_stats': False, 'new': True}], 
#             'unlocked_slugs': ['preamble2', 'round2p1']}

@require_GET
def round(request, slug):
    round = Round.objects.filter(slug=slug).first()
    if round:
        round_puzzles = render_puzzles(request)
        all_puzzles_for_round = get_all_puzzles_for_round(request, round)
        #round_preambles = render_preambles(request)
        if slug in round_puzzles:
            request.context.round = round
            template_name = 'round_bodies/{}.html'.format(slug)
            puzzle_helper = []
            for r in round_puzzles[slug]['puzzles']:
                if "preamble" not in r['puzzle'].name:
                    puzzle_helper.append(r['puzzle'])
            try:
                return render(request, template_name, {'round': round_puzzles[slug], 'all_puzzles': all_puzzles_for_round, 'puzzle_helper': puzzle_helper})
            except (TemplateDoesNotExist, IsADirectoryError):
                # A plausible cause of it being a directory is that the slug
                # is blank.
                return redirect('puzzles')
        # elif slug in round_preambles:
        #     request.context.round = round
        #     template_name = 'round_preambles/{}.html'.format(slug)
        #     try:
...
def get_all_rounds(request):
    all_rounds = Round.objects.select_related().order_by('order')
    return all_rounds

def get_all_puzzles_for_round(request, round):
    puzzles = Puzzle.objects.select_related().filter(round=round).order_by('order')
    return puzzles

def check_team_solved_all_puzzles(request):
    team = request.context.team
    puzzles = Puzzle.objects.select_related().order_by('order')
    
    for puzzle in puzzles:
        submissions = AnswerSubmission.objects.select_related().filter(team=team, puzzle=puzzle, is_correct=True)
...
def render_puzzles(request):
    ...
    for puzzle in request.context.unlocks:
        ...
        data = {'puzzle': puzzle}
        if not isinstance(puzzle, Preamble) and puzzle.id in solved:
            data['answer'] = puzzle.answer
            if puzzle.is_meta:
                rounds[puzzle.round.slug]['meta_answer'] = puzzle.answer
        if not isinstance(puzzle, Preamble) and puzzle.id in hints:
            data['hints'] = hints[puzzle.id]
        data['full_stats'] = full_stats
        if not isinstance(puzzle, Preamble) and puzzle.id in guesses:
            data['solve_stats'] = {
                'correct': correct[puzzle.id],
                'guesses': guesses[puzzle.id],
                'teams': len(teams[puzzle.id]),
            }
        if not isinstance(puzzle, Preamble) and puzzle.id in survey_averages:
            data['survey_stats'] = [{
                'average': average,
...
def preamble(request):
    '''View a single puzzle's content.'''
    team = request.context.team
    preamble = request.context.preamble
    round = preamble.round
    template_name = 'preamble_bodies/{}'.format(request.context.preamble.body_template)
    data = {
    }
    try:
        if not request.context.preamble_completed:
            # print("Preamble completion saved")
            PreambleCompletion(
                    team=team,
                    preamble=preamble,
                ).save()
        ...
        # is blank.
        data['template_name'] = template_name
        return render(request, 'preamble.html', data)


def check_intermediate_confirmation_for_puzzle(request, puzzle, attempt):
    puzzle = request.context.puzzle
    confirmations = [x for x in IntermediateConfirmation.objects.filter(puzzle=puzzle) if attempt.lower() == x.input_text.lower()]
    if len(confirmations) > 0:
        # print("HI")
        return confirmations[0].feedback_text
    else:
        return None
...
def solve(request):
    ...
    if request.method == 'POST' and 'answer' in request.POST:
        ...
        if request.context.guesses_remaining <= 0:
            messages.error(request, _('You have guessed ten times in the past minute! Wait a minute, then resubmit.'))
            return redirect('solve', puzzle.slug)
        ...
        feedback_message = check_intermediate_confirmation_for_puzzle(request, puzzle, normalized_answer)
        ...
        if puzzle_messages:
            ...
            if is_correct:
                unlock_group = puzzle.unlock_group
                solve_record = PuzzleFirstViewTime.objects.select_related().filter(team=team, unlock_group=unlock_group).first()

                if solve_record and unlock_group:
                    solve_record.solve_count += 1
                    if solve_record.solve_count == unlock_group.count:
                        solve_record.fullsolve_time = request.context.now
                        print((solve_record.fullsolve_time - solve_record.first_view_time) / timedelta(milliseconds=1))
                        print(team.time_so_far)
                        team.time_so_far += (solve_record.fullsolve_time - solve_record.first_view_time) / timedelta(milliseconds=1)
                        team.save()
                    solve_record.save()
                else:
                    # something really shitty happened, tell people
                    # this is theoretically impossible
                    messages.error("Something unexpected happened, give organizers this code: E917")

                if not request.context.hunt_is_over:
                    team.last_solve_time = request.context.now
                    team.save()
                messages.success(request, _('%s is correct!') % puzzle.answer)
                if check_team_solved_all_puzzles(request):
                ...
                messages.warning(request, _('%s is on the right path!') % normalized_answer)
            else:
...
def victory(request):
    ...
    if not request.context.hunt_is_over and not request.context.is_superuser:
        ...
        finished = check_team_solved_all_puzzles(request)
        if not finished:
            raise Http404
...
def novice_advanced_middleware(get_response):
    def middleware(request):
        if not (request.path.startswith('/novice/') or request.path.startswith('/advanced/')):
            # print(request.path)            
            response = get_response(request)
            return response
        ...
        team = request.context.team
        # print(team)
        dbteam = Team.objects.select_related().filter(pk=team.pk).first()
        # print(team)
        # print(dbteam)

        if not dbteam:
            messages.error(request, _('Team somehow doesn\'t exist!'))
            response = get_response(request)
            return response

        if dbteam.division != 0:
            messages.error(request, _('You\'ve already placed into a division!'))
            response = get_response(request)
            return response
        ...
        # should never be hit
        # messages.error(request, _('Something went wrong: we\'re investigating.'))
        response = get_response(request)
        return response

    return middleware

def puzzle_view_middleware(get_response):
    def middleware(request):
        if request.path.startswith('/puzzle/'):
            # Extract team and puzzle slug from the request
            team = request.context.team  # Assuming you're using User objects for teams
            slug = request.path.removeprefix('/puzzle/')
            
            # print(slug)
            # print(team)

            puzzle = Puzzle.objects.select_related().filter(slug=slug).first()
            if team and puzzle and puzzle in request.context.unlocks:
                record = PuzzleFirstViewTime.objects.select_related().filter(team=team, unlock_group=puzzle.unlock_group).first()

                # If the record is created (i.e., it didn't exist before), set the first_view_time
                if not record:
                    PuzzleFirstViewTime.objects.create(team=team, unlock_group=puzzle.unlock_group, first_view_time=request.context.now)
                    request.context.first_hint = 0
                    # print("created!")
                else:
                    request.context.first_hint = (request.context.now - record.first_view_time).total_seconds()
